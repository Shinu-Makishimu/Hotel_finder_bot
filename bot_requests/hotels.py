...
def get_hotels(user_id):
    ...
    if params['command'] == 'bestdeal':
        ...
        data = choose_best_hotels(hotels=data['results'], distance=distance, limit=quantity)
    else:
        data = data['results']
    ...
    try:
        if hotels['total_count'] > 0:
            for cur_hotel in data.get('results'):
                ...
                if not hotel['price']:
                    continue
                hotel['distance'] = cur_hotel.get('landmarks')[0].get('distance',
                                                                      interface['errors']['no_information']
                                                                      [get_settings(user_id=user_id, key='language')])
                hotel['address'] = hotel_address(cur_hotel, user_id=user_id)
