...
def commands_catcher(message) -> None:
    """
    отлов команд
    :param message:
    :return:
...
def buttons_catcher_main(call) -> None:
...
def buttons_catcher_settings(call) -> None:
...
def buttons_catcher_money(call) -> None:
...
def buttons_catcher_language(call) -> None:
...
def main_menu(user_id, command, chat_id) -> None:
...
def city_buttons_catcher(call) -> None:
...
def hotel_counter(message) -> None:
...
def photo_counter_answ(message) -> None:
...
def callback_calendar_1(call) -> None:
    # {LSTEP[step]}
...
def callback_calendar_2(call) -> None:
...
def distanse_from_centre(message) -> None:
...
def min_max_price(message) -> None:
...
def end_conversation(user_id, chat_id) -> None:
